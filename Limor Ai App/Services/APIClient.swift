...
struct APIClient {
    ...
    /// Colleagues in the caller's company — for share-with suggestions.
    struct OrgMember: Decodable, Hashable {
        let email: String
        let display_name: String?
    }
    func getOrgMembers() async throws -> [OrgMember] {
        struct Resp: Decodable { let members: [OrgMember] }
        let resp: Resp = try await get("/api/orgs/me/members")
        return resp.members
    }
