...
struct NowView: View {
    ...
    /// Tip ids the user waved off with "not interesting" — mirrored from
    /// SharedStore into @State so the card re-renders on dismissal.
    @State private var dismissedTipIds: Set<String> = SharedStore.dismissedTipIds
    ...
    private var recommendationsCard: some View {
        ...
        if !recs.isEmpty {
            RecommendationsCard(recommendations: recs, onDismissTip: dismissTip)
        }
    }

    /// Hide the tip locally (immediately) + teach the backend the rejected
    /// title so similar angles stop coming back in future batches.
    private func dismissTip(_ rec: Recommendation) {
        UIImpactFeedbackGenerator(style: .light).impactOccurred()
        var ids = SharedStore.dismissedTipIds
        ids.insert(rec.id)
        SharedStore.dismissedTipIds = ids
        withAnimation(.easeInOut(duration: 0.25)) { dismissedTipIds = ids }
        Task { try? await APIClient.shared.rejectTip(title: rec.title) }
    }
...
private struct RecommendationsCard: View {
    ...
    /// "Not interesting" — parent hides the tip + syncs the rejection.
    let onDismissTip: (Recommendation) -> Void
    ...
    var body: some View {
        ...
        VStack(alignment: .trailing, spacing: 12) {
            HStack(spacing: 8) {
                ...
                Text(tr("טיפ אישי מלימור", "A personal tip from Limor"))
                    ...
                    .foregroundStyle(.limorInk)
                Spacer()
                // "Not interesting" — removes the CURRENT tip. Positioned in
                // the header (far edge) so a stray tap on the tip body opens
                // the detail sheet instead of deleting.
                Button {
                    let current = recommendations.indices.contains(index)
                        ? recommendations[index] : recommendations[0]
                    onDismissTip(current)
                    if index >= recommendations.count - 1 { index = 0 }
                } label: {
                    Image(systemName: "xmark.circle.fill")
                        .font(.subheadline)
                        .foregroundStyle(.limorMuted.opacity(0.6))
                }
                .buttonStyle(.plain)
                .accessibilityLabel(tr("הסר את הטיפ", "Dismiss this tip"))
            }
        ...
        // (The decorative corner sparkle was dropped — the "not interesting"
        // ❌ now lives in that corner of the header.)
        ...
        .sheet(item: $selectedTip) { tip in
            TipDetailView(recommendations: recommendations, initial: tip, onDismissTip: { rejected in
                selectedTip = nil
                onDismissTip(rejected)
                if index >= recommendations.count - 1 { index = 0 }
            })
            .presentationDetents([.medium, .large])
...
private struct TipDetailView: View {
    ...
    /// "Not interesting" — bubbles up so the parent hides the tip and
    /// teaches the backend the rejected angle.
    let onDismissTip: (Recommendation) -> Void
    @EnvironmentObject private var router: AppRouter
    @Environment(\.dismiss) private var dismiss

    init(recommendations: [Recommendation], initial: Recommendation, onDismissTip: @escaping (Recommendation) -> Void) {
        self.recommendations = recommendations
        _current = State(initialValue: initial)
        self.onDismissTip = onDismissTip
    }
                    ...
                    VStack(spacing: 10) {
                        ...
                        Button {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(tr("לא מעניין אותי — אל תציעי שוב", "Not interesting — don't suggest again"))
                            }
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(.limorMuted)
                            .padding(.vertical, 8)
